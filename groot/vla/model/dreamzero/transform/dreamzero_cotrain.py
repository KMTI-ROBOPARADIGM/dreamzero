...
def collate(features: List[dict], tokenizer: AutoTokenizer, num_views=3, embodiment_tag_mapping=None) -> dict:
    ...
    for key in keys:
        if key == "text":
            # ... [Keep your existing text/tokenizer logic here] ...
            output_values = []
            for elem in features:
                item = elem[key]
                try:
                    parsed_item = ast.literal_eval(item)
                    processed_item = str(parsed_item[0]) if isinstance(parsed_item, (list, tuple)) else str(parsed_item)
                    # ... [Keep embodiment prompt logic] ...
                    output_values.append(processed_item)
                except (ValueError, SyntaxError, TypeError):
                    output_values.append(str(item).lower())
            ...
            # ... [Keep your existing text_negative logic] ...
...
class DreamTransform(InvertibleModalityTransform):
        ...
        for key in data.keys():
            try:
                ...
                if "annotation" in key:
                    modality = "language"
            except ValueError:  # Handles unpack error without broad except
    ...
    def _prepare_action(self, data: dict):
        ...
        assert n_action_tokens % self.action_horizon == 0, (
